...
also merge (FIXME: Do we theoretically need a check for a post-dominator? This
doesn't seem to happen(?) in "normal" CFG but could happen in weirder cases. 
Not sure if this is a practical issue though, because it's safe either way and
if we enable further optimizations the movement is probably worth it either way).
